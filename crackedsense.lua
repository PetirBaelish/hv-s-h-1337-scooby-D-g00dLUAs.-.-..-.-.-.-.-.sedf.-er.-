...
notify.new_bottom(179, 255, 18, { { 'Thanks for downloading!' }, { "... ", true }, })
...
return (function(tbl)
    ...
    local prefix = function(x, z) 
        return (z and ("\a32a852FFcrackedsense \a698a6dFF~ \a414141FF(\ab5b5b5FF%s\a414141FF) \a89f596FF%s"):format(x, z) or ("\a32a852FFcrackedsense \a698a6dFF~ \a89f596FF%s"):format(x)) 
    end
    ...
    menu = {
        ["anti aimbot"] = {
            ...
            ["features"] = {
                legit = ui.new_combobox("aa", "anti-aimbot angles", prefix("legit"), {"off", "default", "crackedsense"}),
        ...
        ["visuals & misc"] = {
            ...
            ["visuals"] = {
                ...
                arrows = ui.new_combobox("aa", "anti-aimbot angles", prefix("arrows"), {"-", "simple", "body", "crackedsense"}),
        ...
        ["config"] = {
            export = ui.new_button("aa", "anti-aimbot angles", "\a89f596FF export", function()
                ...
                clipboard.export(json.stringify({["crackedsense"] = tbl}))
            end),
            import = ui.new_button("aa", "anti-aimbot angles", "\a32a852FF import", function()
                local check, message = pcall(function()
                    ...
                    for i, v in next, tbl["crackedsense"]["extra"] do
                        if i == "submenu" then
                    ...
                    tbl["crackedsense"]["extra"] = nil
                    for i, v in next, tbl["crackedsense"] do
                        for index, value in next, v do
                            for ii, vv in next, value do
                                if ii == "type" then
    ...
    for i, v in next, tbl.states do
        ...
        for index, value in next, {"ct", "t"} do
            aa[v][value] = {
                ["type"] = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "type"), (v == "global" and {"normal", "crackedsense", "advanced", "auto"} or {"disabled", "normal", "crackedsense", "advanced", "auto"})),
                ["normal"] = {
                    ...
                    method = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "method"), {"default", "crackedsense"}),
                    ...
                    body = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "body"), {"off", "crackedsense", "opposite", "static", "jitter"}),
                    ...
                    defensive = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "defensive"), {"off", "always on", "lcrackedsense"})
                },
                ["lcrackedsense"] = {
                    crackedsense = ui.new_slider("aa", "anti-aimbot angles", prefix(v .. " " .. value, "crackedsense"), 1, 10, 1),
	...
	tbl.callbacks = {
        ...
        ["command"] = function(arg)
            ...
            if ui.get(menutbl["type"]) == "normal" then
                ...
                if tbl.contains(ui.get(menutbl["mode"]), "left right") then
                    ...
                    if ui.get(menutbl["method"]) == "crackedsense" then
                        method = arg.chokedcommands ~= 0
                    end
                    if method and ui.get(menutbl["body"]) ~= "crackedsense" then
                ...
                if ui.get(menutbl["body"]) ~= "crackedsense" then
                ...
                if ui.get(menutbl["defensive"]) == "crackedsense" then
            ...
            elseif ui.get(menutbl["type"]) == "crackedsense" then
                menutbl = menutbl[ui.get(menutbl["type"])]
                ui.set(tbl.items.jitter[1], "off")
                ui.set(tbl.items.body[1], "static")
                if arg.command_number % (ui.get(menutbl["crackedsense"])+1+1) == 1 then
					tbl.antiaim.ready = true
                end
				if tbl.antiaim.ready and arg.chokedcommands == 0 then
			...
			if tbl.normal_aa and tbl.reset_aa and ui.get(menu["anti aimbot"]["features"]["fixer"]) == "crackedsense" then
				tbl.reset_aa = false
				for i = 1, 69 do
                            ...
                            for iii, vvv in next, vv do
                                ...
                                if ii == "normal" then
                                    ...
                                    if iii == "body_slider" then
                                        fix = ui.get(vv["body"]) ~= "off" and ui.get(vv["body"]) ~= "opposite" and ui.get(vv["body"]) ~= "crackedsense"
                                    end
                                    if iii == "custom_slider" then
                                        fix = ui.get(vv["body"]) == "crackedsense"
                                    end
                                    if iii == "yaw" then
                                ...
                                if ii == "auto" then
                                    if iii == "timer" or iii == "left" or iii == "right" then
                                        fix = ui.get(vv["method"]) == "crackedsense"
                                    end
                                end
        ...
        ["arrows"] = function()
            ...
            if myself ~= nil and entity.is_alive(myself) then
                ...
                elseif ui.get(menu["visuals & misc"]["visuals"]["arrows"]) == "crackedsense" then
                    local xv, yv, zv = entity.get_prop(myself, "m_vecVelocity")
		...
		["indicator"] = function()
			...
			if ui.get(menu["visuals & misc"]["visuals"]["indicators"]) == "default" then
				...
				if yaw_body > 0 then
					renderer.text( w, h + ind_height, 255, 255, 255, 255, "cb", nil, gradient(r2, g2, b2, a2, r1, g1, b1, a1, "crackedsense") )
				else
					renderer.text( w, h + ind_height, 255, 255, 255, 255, "cb", nil, gradient(r1, g1, b1, a1, r2, g2, b2, a2, "crackedsense") )
				end
